document.addEventListener('DOMContentLoaded', function() {
    ...
    searchButton.addEventListener('click', function(event) {
        ...
        if (searchTerm !== '') {
            window.location.href = `SearchPage.html?term=${encodeURIComponent(searchTerm)}`;
        }
    });
    ...
    const SEARCH_URL = `http://127.0.0.1:8081/search?term=${encodeURIComponent(searchTerm)}`;
